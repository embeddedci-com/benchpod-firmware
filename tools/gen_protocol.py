...
OPCODES = [
    ...
    ("START_DAC",       0x11, "period(2) + divider(2)",                         "— (shallow: loops the 4 KB LOAD_WAVE BRAM, <=2048 samples. A DAC sample takes max(divider, 3) + 51 clk48: v2 >=v34 floors the divider at 3 for the DAC8551's frame-gap timing (t9), earlier gateware at 0)"),
    ("STOP_DAC",        0x12, "—",                                              "—"),
    ...
    ("SET_DAC_STOP_AFTER", 0x14, "cycles(4)",                                    "— (v2 >=v21; arms the iCE40 to cut a concurrently-running DAC `cycles` of the 24 MHz clk after the NEXT capture's t0, so the captured window shows the DAC switch off; cycles=0 disarms; send BEFORE the capture-start command)"),
    ...
    ("START_CAPTURE",   0x20, "count(2) + divider(2)",                          "— (16-bit shallow ADC; deep ADC uses CAPTURE 0x31 with la_cnt=0. CAPTURE DIVIDERS (here, START_MEASURE cap_div, CAPTURE adc_div/la_div, LA_CAPTURE) are the sample PERIOD in 24 MHz clk: rate = 24 MHz / divider EXACTLY on >=v32; ADC floors it at 60, LA at 2. Gateware <=v31 sampled every divider+1 clk — firmware sends divider-1 there.)"),
    ...
    ("START_MEASURE",   0x30, "count(2) + dac_div(2) + cap_div(2)",             "— (DAC preloaded; 16-bit count; cap_div is a capture divider — see START_CAPTURE; dac_div is not: a DAC sample takes max(dac_div, 3) + 51 clk48, see START_DAC)"),
    ("CAPTURE",         0x31, "adc_cnt(3)+adc_div(2)+la_cnt(3)+la_div(2)",      "— (v2; unified ADC+LA, one trigger, two PSRAM regions; both 24-bit deep; divider = sample period, see START_CAPTURE)"),
    ...
    ("LA_CAPTURE",      0x69, "count(3) + divider(2)  (24-bit samples; 2 B/sample)", "— (v2; deep PSRAM stream up to 8 MB, MCU reads back over XSPI; divider = sample period, see START_CAPTURE)"),
    ("UART_CONFIG",     0x70, "rx_ch+tx_ch+div(3)+flags",                       "—"),
    ("UART_DISABLE",    0x71, "—",                                              "—"),
